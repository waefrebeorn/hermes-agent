# Plan — Next Phase (v415)

S0+S1+S3+S6+R02+R04+R10 PORTED. F10 PORTED. 68 gaps. Suite 328/0/12.
...
|----------|--------|-----|--------|
| P1 | S5 C11 | Auth/OAuth CLI | Add interactive `/auth login <provider>` with browser-open callback flow, reusing libmcp_oauth infrastructure. Currently `/auth status|providers` done. Missing: interactive login. |
