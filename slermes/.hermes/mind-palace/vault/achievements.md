...
>
|> **v397** · 68 active gaps · **2154 entries** of progress
...
|| 340 | X09 | Clarify tool edge case expansion — 12 new test functions (12→24 tests, 29 new assertions 12→41). Covers: empty choices array (no choices_offered), single choice, out-of-range index 0 (atoi-1 negative), negative index, quotes in response (JSON-special chars), whitespace-only response (3 spaces preserved), skip with choices, very long question (1100 chars), null choice items gracefully skipped (not displayed, not selectable). | `tests/test_clarify.c` — 12 new test sections (12→24 tests). Suite 326/0/14. v397. |
|
