...
|| 377 | S4 T02 | TUI Transport Layer — FIFO-based transport abstraction encapsulating connection state machine (IDLE/CONNECTING/CONNECTED/DISCONNECTED/ERROR), configurable reconnection (max_retries + delay_ms), state change callbacks, newline-delimited message framing with poll-based I/O, send/recv/sendf/send_rpc API, shutdown with FIFO cleanup. Uses O_RDWR for self-consistent FIFO lifecycle (no reader requirement). Replaces ad-hoc tui_gateway_init/connect/poll/send in tui_fullscreen.c. T02 changes from None to PORTED. S4 gaps: 12->11. Total: 59->58. Suite 332/0/13. v433. | src/cli/tui_transport.c/h -- new files, +680 LOC, full API: tui_transport_init(), tui_transport_connect(), tui_transport_reconnect(), tui_transport_poll(), tui_transport_recv(), tui_transport_send(), tui_transport_sendf(), tui_transport_send_rpc(), tui_transport_is_connected(), tui_transport_state(), tui_transport_state_name(), tui_transport_set_retry(), tui_transport_set_callback(), tui_transport_shutdown(). tests/test_tui_transport.c -- +370 LOC, 13 test functions (init, init null path, connect, state names, set_retry, send_recv roundtrip, reconnect, state callback, send_rpc, sendf, poll empty, recv nonblock, send after shutdown). test_runner.sh -- entry added. Makefile -- TUI_SRC/OBJ extended. |
|| 378 | S4 T08 | App Layout + Chrome — configurable pane layout engine with responsive modes (NORMAL/MOBILE/COMPACT/WIDE), pane sizing policies (FIXED/RATIO/FILL/AUTO), pane constraints (min/max), side-based placement (TOP/BOTTOM/LEFT/RIGHT/CENTER), chrome flags (HEADER/BORDER/SEPARATOR/SCROLL/FOOTER). Replaces hardcoded inline resize math in tui_fullscreen.c (~80 LOC). T08 changes from ncurses-panel-only to PORTED. S4 gaps: 11->10. Total: 58->57. Suite 333/0/13. v434. | src/cli/tui_layout.c/h -- new files, +680 LOC, full API: tui_layout_init(), tui_layout_set_mode(), tui_layout_add_pane(), tui_layout_set_chrome(), tui_layout_set_title(), tui_layout_calculate(), tui_layout_resize(), tui_layout_find_pane(), tui_layout_pane_visible(), tui_layout_mode_name(), tui_layout_get_pane(), tui_layout_pane_count(). tests/test_tui_layout.c -- +320 LOC, 15 test functions (init, init defaults, add pane, max panes, calculate center, calculate top/bottom, calculate left/right, mode names, set mode, resize, find pane, set chrome/title, pane count/get, zero height visibility, ratio sizing). test_runner.sh -- entry added. Makefile -- TUI_SRC/OBJ extended. |
|| 379 | S4 T03 | TUI Render Engine — virtual screen with dirty-rect tracking (per-cell dirty flag, bounding box min/max), double buffering via deferred cell updates, text wrapping at surface width with automatic scrolling when cursor exceeds bottom, color/attribute stacks (tui_render_set_color/set_attrs), full markdown render pipeline supporting **bold**, `code` (inline), `#` headers (role-colored), indented code blocks (dim pair), role-based coloring (8 roles: DEFAULT/USER/ASSISTANT/SYSTEM/TOOL/INFO/ERROR/DEBUG). Flush to ncurses with incremental dirty-region update (wmove + wattron/waddch per changed cell). 8 role color pairs. T03 changes from ncurses-panel-only to PORTED. S4 gaps: 10->9. Total: 57->56. Suite 334/0/13. v435. | src/cli/tui_render.c/h -- new files, +810 LOC, public API: tui_render_new/free/resize/clear, set_color/set_attrs/set_align, putchar/write/printf/newline/blank_lines/hr, render_markdown, mark_all_dirty/flush, cursor queries. tests/test_tui_render.c -- +380 LOC, 16 test functions (create, write cursor, wrapping, newline, scrolling, clear, resize, color/attrs, hr, blank_lines, markdown basic, markdown formatting, markdown code block, role colors, printf, carriage return). test_runner.sh -- entry added (with ncurses include + unresolved-symbols-ignore). Makefile -- TUI_SRC/OBJ extended. |
||| 380 | S4 T04 | TUI WebSocket Support — WebSocket server wrapper for TUI backend: tui_ws_start(port, cert, key), tui_ws_port(), tui_ws_stop(), tui_ws_accept(timeout_ms), tui_ws_send(), tui_ws_recv(), tui_ws_client_close(), tui_ws_is_connected(). libwebsocket extended with server API: ws_server_listen/accept/close/port, ws:// plain connections supported (was wss:// only), write_raw/read_raw static helpers for SSL/plain auto-dispatch. T04 changes from None to PORTED. S4 gaps: 9->8. Total: 56->55. Suite 335/0/13. v436. | src/cli/tui_websocket.c/h -- new files, +300 LOC. lib/libwebsocket/websocket.c/h -- extended: +170 LOC for server functions, ws:// parse/connect support, write_raw/read_raw helpers. tests/test_tui_websocket.c -- +120 LOC, 4 test functions (start_stop, accept_nonblock, port_zero, restart). test_runner.sh -- entry added (with libwebsocket dep + ssl/crypto). Makefile -- TUI_SRC/OBJ extended. |
| 381 | S7 X09 | TUI Edge Case/Regression Tests — 43 edge case tests across 5 TUI modules. json_rpc (13): empty/whitespace/whitespace-only input, huge positive ID (999999999), negative ID treated as notification, null ID, string ID, array params, missing params, batch array requests, duplicate method registration idempotent, 50-method registration stress, 255-char method name, wrong-type params. transport (8): NULL init pointer safe, double init no-op, zero-length send ("" and NULL,0), 65536-byte large send, non-blocking recv (timeout 0), send before connect returns false, poll on disconnected returns -1, reconnect after shutdown fails gracefully. layout (8): zero-dim init clamped to 24x80, 1000x1000 extreme dimensions handled, negative dimensions (no crash), 100-char pane name truncated to 31+null, all-chrome rendering (CHROME_ALL), operations on empty layout (find/visible/count/get all safe), out-of-bounds pane index (set_chrome, set_title, get_pane, visible all safe), add after TUI_LAYOUT_MAX_PANES returns -1. render (9): zero-dim surface clamped to 1x1, 10000x10000 huge dims (OOM safe), NULL string/char write, 100K-char extreme string, 500-line scrollback overflow (cursor stays at last row), extreme color values (-100, 10000, 65535), alignment switching (CENTER→RIGHT→LEFT), negative blank line count, edge markdown inputs (empty, unclosed bold, backtick mismatch, bare #, just indent, bare **). websocket (5): port 0 auto-assign, single stop (no double-free), accept on valid server non-blocking returns NULL, negative port (-1 passes through), huge port (99999 passes through). S7 X09 changes from ~5% to ~74% C/Python edge test parity. S7 clusters: 19->18. Total: 55->54. Suite 336/0/13. v437. | tests/test_tui_edge.c -- new file, ~600 LOC, 43 test functions. test_runner.sh -- entry added (links tui_json_rpc/transport/layout/render/websocket + libwebsocket + json + ssl). |
| 382 | S7 X08 | Conversation Loop Edge Case Tests — 29 edge case tests expanding conversation loop coverage. Message repair (9): 4-tool assistant with all valid results, mixed valid + stray tool calls (only strays dropped), orphan tool message without preceding assistant, system+user consecutive (valid), system+2user consecutive (merged correctly), tool result with NULL content but valid ID (preserved), interleaved assistant/tool/assistant/tool sequences, special characters in tool call IDs ($!-_123), all-stray tool messages (all dropped, count→0). Tool call argument sanitization (4): nested JSON objects as args (valid), JSON array as args (valid), escaped string with quotes/newlines (valid), max 8 tool calls all valid. Agent loop infrastructure (3): max_iterations field existence in agent_state_t, msg_role_t enum value ordering sanity, 2KB+ tool call arguments string (valid JSON). S7 X08 changes from ~5% to improved conversation loop edge parity. S7 clusters: 18 (X08 depth improved). Total: 54->53. Suite 337/0/13. v438.
| 383 | S7 X04 | Memory Tool Edge Case Tests — 39 new edge case assertions (13->52 total) in test_memory.c. Coverage: count on empty memory, get with existing key/non-existent key/NULL key/empty key, delete with NULL key/empty key, search with NULL query/empty query/limit=0, clear on empty memory/after use/NULL pointer safety, re-store after clear, entry_expired with no expiry/past expiry/future expiry, hash_content deterministic/unique/empty/NULL, get_prioritized ordering (highest priority first), 5000-char content store, duplicate key update semantics (content and priority overwritten), negative priority store, double cleanup safety, NULL-safe API for clear/count/list_keys. S7 X04 changes from 25% to improved memory tool edge parity. S7 clusters: 18 (X04 depth improved). Total: 53->53 (depth improved, no new gaps closed). Suite 337/0/13. v439. | tests/test_memory.c -- expanded 13->52 assertions. | | tests/test_conversation_edge.c -- new file, ~380 LOC, 16 test functions (29 assertions). test_runner.sh -- entry added (links agent_message_repair + sanitize + redact + json). |
| 384 | S7 X06 | Agent Loop Core Function Tests — 90 new assertions in test_agent_loop.c. 6 test groups, 15 test functions. Coverage: session_id format (8: non-empty, length 15, regex match YYYYMMDD_HHMMSS, year/month/day/hour/min/sec ranges), agent_free — zeroed state (4: no crash, messages NULL, count 0, capacity 0), agent_free with messages (4: no crash, cleanup), agent_configure_from_config — full mapping (11: model/provider/base_url/api_key/max_tokens/temperature/top_p/max_iterations/tool_delay/compress_enabled/cooldown), partial config (8: model set, other fields stay zero/default), NULL safety (3: NULL state, NULL config, both), agent_inject_history — valid JSON (8: system/user/assistant roles + content), NULL/empty (5: NULL/empty/[]/NULL state), invalid JSON (5: object/string/number/truncated/no-change), tool role (3: tool+assistant messages), append (5: first-second-preserve-order), agent_snapshot — empty state (4: no crash, count 0, capacity allocated, restore returns false), with messages (10: 2 snapshots, content preserved, pointers differ, modify then restore), double take (3: old freed, new captured), agent_free with snapshot (4: messages freed, snapshot NOT freed by agent_free — known pre-existing behaviour documented). S7 X06 changes from ~20% to improved agent loop core test parity. S7 clusters: 18 (X06 improved, X04 improved, X09 ported). Total: 53->53 (depth improved, no new gaps). Suite 338/0/13. v440. | tests/test_agent_loop.c -- new file, ~650 LOC, 15 test functions (90 assertions). test_runner.sh -- entry added (links agent_loop.c + context.c + checkpoint.c + json.c + plugin.c). |
| 385 | S7 X07 | Gateway Platform Test Expansion — 48 new assertions in test_gateway_platforms.c (75→123 total). Coverage: WhatsApp setters (4: set_token, set_phone_id, set_verify_token, re-set), WhatsApp webhook verification (9: valid challenge response, wrong token rejected, wrong mode rejected, missing challenge/token/mode params NULL/empty/no-verify-token), Slack setters (4: token, channel, signing_secret, re-set), Slack get_chat_id (2: returns static channel_id, NULL-safe), Matrix setters (8: homeserver, token, room, user_id, event_filter, empty filter, NULL filter, re-set), Matrix get_chat_id (3: chat room ID from JSON, NULL safe, no-message returns NULL), Matrix get_text (4: text from message, NULL safe, no-message returns NULL, no-text-field returns NULL), Mattermost setters (6: URL, token, channel, trailing slash stripped, multiple trailing slashes, re-set), Discord get_chat_id (4: channel_id from message, NULL safe, fallback to static channel_id when no message), Discord get_text (4: text from message, NULL safe, no-message returns NULL, no-text-field returns NULL). S7 X07 changes from ~22% to improved gateway platform test parity. S7 clusters: 18 (X07 improved, X06 improved, X04 improved, X09 ported). Total: 53->53 (depth improved, no new gaps). Suite 338/0/13. v441. | tests/test_gateway_platforms.c — expanded 75→123 assertions (+48). test_runner.sh — updated compile line (added slack.c, matrix.c, mattermost.c, whatsapp.c), count 75→123. |
| 386 | S7 X08 | Conversation Loop Edge Case Expansion — 19 new assertions in test_conversation_edge.c (29→48 total). Coverage: consecutive assistants (2: no crash, count does not increase), assistant before system (1: no crash), duplicate tool call ID (1: no crash), system only (2: no crash, count unchanged), assistant only (1: no crash), tool only (2: tool dropped, count→0), null tool name in assistant (1: repair succeeds with empty tool name), long tool ID (2: 59-char ID matched, count preserved), NULL safety (7: repair/sanitize with NULL msgs, NULL count, combined NULL; sanitize NULL count with valid msgs), negative count (1: no crash), zero count (1: no crash). S7 X08 changes from ~5% to improved conversation loop edge case parity. S7 clusters: 18 (X08 improved, X07 improved, X06 improved, X04 improved, X09 ported). Total: 53->53 (depth improved, no new gaps). Suite 338/0/13. v442. | tests/test_conversation_edge.c — expanded from 29→48 assertions (+19), 11 new test functions, ~220 new LOC. test_runner.sh — count 18→48. |
| 387 | S7 X08 | Conversation Loop Edge Case Expansion (Round 2) — 28 new assertions in test_conversation_edge.c (48→76 total). Coverage: hermes_message_sanitize tests (7: NULL msg, empty msg, think block stripping, think no-op, api_key redaction with max_show=4, surrogate cleanup, tool args redaction). message_free (3: NULL safe, empty msg, content cleanup). Additional repair (6: 3 consecutive users merged, cross-assistant tool IDs valid, empty tool ID no crash, user after tool valid, mixed roles large valid, all tool IDs mismatched dropped). S7 X08 depth improved. S7 clusters: 18 (X08 depth improved). Total: 53->53 (depth improved, no new gaps). Suite 338/0/13. v443. | tests/test_conversation_edge.c — expanded 48→76 assertions (+28), 16 new test functions, ~410 new LOC. test_runner.sh — added agent_message_sanitize.c + context.c to compile line, count: 48→76. |
| 388 | S7 X10 | Fuzz Test Expansion (Round 2) — 5 new assertions in test_fuzz.c (15→20 total). Coverage: HTML: event handlers, nested scripts, style/conditional comments, 100-level deep nesting (test_html_malformed +2 cases, test_html_extreme_nested). Path: home dir, over-traversal, Windows/UNC, 4096-char path (test_path_malformed +4 cases, test_path_very_long). Regex: escaped bracket, alternation, realistic patterns, email regex (test_regex_more_edge 5 patterns). Template: lone endif, unclosed if/var/unknown tag (test_template_unmatched_tags 4 inputs). JSON: numeric edge cases, 50-level recursive array (test_property_json_numeric_edge 6 inputs, test_property_json_recursive_array). S7 X10 depth improved. S7 clusters: 18 (X10 improved, X08 improved, X07 improved, X06 improved, X04 improved, X09 ported). Total: 53->53 (depth improved, no new gaps). Suite 338/0/13. v444. | tests/test_fuzz.c — expanded 15→20 assertions (+5), 7 new test functions, ~190 new LOC. test_runner.sh — label updated. |
| 389 | S7 X04 | Process Tool Edge Case Expansion — 8 new assertions in test_process.c (26→34 total). Coverage: negative session_id (poll returns error), empty command (no crash), write stdin (cat echo, status=written), close stdin (no crash), signal by number 15/SIGTERM (status=signal_sent), log non-existent session (error). S7 X04 depth improved. S7 clusters: 18 (X04 improved, X10 improved, X08 improved, X07 improved, X06 improved, X09 ported). Total: 53->53 (depth improved, no new gaps). Suite 338/0/13. v445. | tests/test_process.c — expanded 26→34 assertions (+8), 5 new test cases, ~70 new LOC. test_runner.sh — label: 26→34. |
| 390 | S7 X04 | Tool Coercion Number Edge Cases — 6 new assertions in test_tool_coerce.c (36→42 total). Coverage: just minus/plus/decimal/minus-decimal all rejected, leading zeros (00042→42) accepted, NULL output pointer safety, NULL is_int pointer safety. S7 X04 depth improved (coercion edge cases). S7 clusters: 18 (X04 improved, X10 improved, X08 improved, X07 improved, X06 improved, X09 ported). Total: 53->53 (depth improved, no new gaps). Suite 338/0/13. v446. | tests/test_tool_coerce.c — expanded 36→42 assertions (+6), ~30 new LOC. |
| 391 | S7 X04 | Delegate Tool Edge Case Expansion — 6 new assertions in test_delegate.c (7→13 total). Coverage: null goal value returns error, empty subtasks array valid, 2000-char goal no crash, negative max_concurrent_children handled, unicode \\u2603\\u20e0 in goal valid, goal+context without subtasks valid. S7 X04 depth improved (delegate edge cases). S7 clusters: 18 (X04 improved, X10 improved, X08 improved, X07 improved, X06 improved, X09 ported). Total: 53->53 (depth improved, no new gaps). Suite 338/0/13. v447. | tests/test_delegate.c — expanded 7→13 assertions (+6), 6 new test cases, ~55 new LOC. test_runner.sh — label: 7→13. |
| 392 | S7 X03 | OpenAI Provider Test Expansion — 74 new assertions in test_provider_openai.c (37→111 total). New coverage: multi-tool calls (3 tool calls, finish_reason=tool_calls), tool call with no arguments (empty {}), null content without tool calls, streaming edge cases (whitespace delta, role delta, reasoning_content in delta, finish_reason=length, finish_reason=content_filter, event+DONE multi-line), empty/missing/null choices array, URL building edge cases (trailing slash with no double-slash, custom proxy path, empty base), header edge cases (empty API key, sk- prefix verification), build_request_body (system+user messages, model name, JSON output, tools in request body). Bug fixes in src/agent/provider_openai.c: parse_response now extracts finish_reason from choice level (was missing), parse_stream_chunk now extracts reasoning_content from delta (was missing). S7 X03 depth improved (OpenAI provider tests 15%→improved). S7 clusters: 18 (X03 improved, X04 improved, X10 improved, X08 improved, X07 improved, X06 improved, X09 ported). Total: 53->53 (code fixes + depth improved). Suite 338/0/13. v448. | tests/test_provider_openai.c — expanded 37→111 assertions (+74), 8 new test functions, ~350 new LOC. src/agent/provider_openai.c — 2 bug fixes (finish_reason extraction in parse_response, reasoning_content extraction in parse_stream_chunk). test_runner.sh — label: 54→111. |
| 393 | S7 X03 | Anthropic Provider Test Expansion — 70 new assertions in test_provider_anthropic.c (28→98 total). New coverage: response parsing — text content (4: Hello! I am Claude, input/output tokens, free), tool_use (5: text preserved, tool count, id, name, args), multi-tool (4: 2 tools, names, args), only tool_use (3: count, name, args not null), empty text (2: parsed, zero-length), null content (1: no crash), invalid content type (1: no crash), error response (2: error message matches, authentication type), malformed (4: empty JSON, invalid JSON, NULL body, empty string), NULL free (1: no crash). Streaming — text delta SSE (3: content extracted, partial word), event stream (7: block_start text, block_start tool_use, block_stop, message_delta end_turn, message_delta tool_use, message_start, ping), edge cases (8: direct data: line, raw JSON, [DONE], NULL chunk, empty chunk, whitespace, event-only). URL edge cases (6: standard v1 base, trailing slash, custom proxy, already has /messages, NULL base, empty base). Header edge cases (3: NULL key, OAuth token, cached). build_request_body deferred (provider registration pipeline issue, no crash). S7 X03 depth improved (Anthropic provider tests). S7 clusters: 18 (X03 improved, X04 improved, X10 improved, X08 improved, X07 improved, X06 improved, X09 ported). Total: 53->53 (depth improved, no new gaps). Suite 338/0/13. v449. | tests/test_provider_anthropic.c — expanded 28→98 assertions (+70), 15 new test functions, ~700 new LOC. test_runner.sh — label: 74→98. |
| 394 | S7 X03 | DeepSeek Provider Test Expansion — 35 new assertions in test_provider_deepseek.c (71→106 total). New coverage: URL edge cases (4: trailing slash no double, proxy path preserved, empty base, free), header edge cases (3: empty key, negative cache TTL via deepseek_cache_ttl=-1, non-crash), tool call parsing (4: parsed, id, name, args Beijing), reasoning+tool combined (4: parsed, reasoning present, tool count, tool name correct), streaming edge cases (8: length finish reason, content_filter finish reason, role delta, content+reasoning both extracted, reasoning text, whitespace delta, empty string, all non-crash), FIM response edge cases (4: empty text, no choices, empty JSON, broken JSON — all non-crash), FIM body edge cases (5: empty prompt, empty suffix, 2000-char prompt, zero max_tokens — all non-crash, memory safe), response empty choices (3: empty array, no key, null — all non-crash). S7 X03 depth improved (DeepSeek provider tests). S7 clusters: 18 (X03 improved, X04 improved, X10 improved, X08 improved, X07 improved, X06 improved, X09 ported). Total: 53->53 (depth improved, no new gaps). Suite 338/0/13. v450. | tests/test_provider_deepseek.c — expanded 71→106 assertions (+35), 8 new test functions, ~300 new LOC. test_runner.sh — label: 60→106. |
| 395 | S7 X03 | Google Provider Test Expansion — 87 new assertions in test_provider_google.c (65→152 total). 8 new test functions: test_url_edge_cases (stream endpoint passthrough, proxy path, empty model → default), test_header_edge_cases (empty key omits x-goog-api-key, NULL key omits, long key included), test_parse_response_finish_reasons (all 9 Google finish reasons: STOP→stop, MAX_TOKENS→length, SAFETY→content_filter, RECITATION→content_filter, OTHER→stop, BLOCKLIST→content_filter, PROHIBITED_CONTENT→content_filter, SPAM→content_filter, IMAGE_SAFETY→content_filter), test_parse_response_content_blocked (SAFETY→[Content blocked], BLOCKLIST→[Content blocked], PROHIBITED_CONTENT→[Content blocked], all with finish_reason=content_filter), test_parse_response_no_candidates (empty array safe, no candidates key with usage tokens, candidate no content safe, empty parts → NULL content), test_is_native_base_url (7 cases: standard, trailing slash, /openai compat→false, custom→false, other domain→false, NULL→false, empty→false), test_google_coerce_content_to_text (10: NULL→"", string→copy, empty string→"", array of strings→joined with \\n, text type object→text field, empty array→""), test_parse_stream_finish_reason_depth (SAFETY→content_filter, MAX_TOKENS→length, SPAM→content_filter), test_parse_stream_empty_candidates (empty array safe, usage only metadata captured, finish reason extracted even without content). S7 X03 depth improved (Google provider tests). S7 clusters: 18 (X03 improved, X04 improved, X10 improved, X08 improved, X07 improved, X06 improved, X09 ported). Total: 53->53 (depth improved, no new gaps). Suite 338/0/13. v451. | tests/test_provider_google.c — expanded 65→152 assertions (+87), 8 new test functions, ~700 new LOC. test_runner.sh — label: 64→152. |
| 396 | S7 X03 | xAI Provider Test Expansion — 69 new assertions in test_provider_xai.c (64→133 total). 6 new test functions: test_url_edge_cases (double-slash fix, proxy path), test_header_edge_cases (empty/NULL key omits Bearer, long key included), test_parse_response_edge_cases (empty choices, no choices key, null content→empty, length finish, no usage), test_parse_response_encrypted (visible text preserved, encrypted_content set/serialized), test_parse_stream_edge_depth (empty delta, empty chunk, length finish, encrypted stream, whitespace), test_model_retirement_all (all 8 retired models: grok-4-0709→grok-4.3, grok-4-fast-reasoning→grok-4.3, grok-4-fast-non-reasoning→grok-4.3+none, grok-4-1-fast-reasoning→grok-4.3, grok-4-1-fast-non-reasoning→grok-4.3+none, grok-code-fast-1→grok-4.3, grok-3→grok-4.3, grok-imagine-image-pro→grok-imagine-image-quality). Bug fix: xai_parse_response — missing finish_reason extraction from choice level (streaming path had it, non-streaming didn't). S7 X03 depth improved (xAI provider tests). S7 clusters: 18 (X03 improved, X04 improved, X10 improved, X08 improved, X07 improved, X06 improved, X09 ported). Total: 53->53 (depth improved, no new gaps). Suite 338/0/13. v452. | tests/test_provider_xai.c — expanded 64→133 assertions (+69), 6 new test functions, ~450 new LOC. src/agent/provider_xai.c — bug fix (+3 lines). test_runner.sh — label: 63→133. |
| 397 | S7 X03 | Azure Provider Test Expansion — 40 new assertions in test_provider_azure.c (54→94 total). 4 new test functions: test_url_edge_cases (proxy URL, custom model, empty base→default azure.com), test_header_edge_cases (empty/NULL key omits api-key, long key included), test_parse_response_edge_cases (empty choices, no choices, null content→empty, length finish+finish_reason, no usage→zero tokens), test_parse_stream_edge_depth (empty delta, empty chunk, length finish reason, whitespace). Bug fix: azure_parse_response — missing finish_reason extraction from choice level (same pattern as xAI Phase 396). S7 X03 depth improved (Azure provider tests). S7 clusters: 18 (X03 improved, X04 improved, X10 improved, X08 improved, X07 improved, X06 improved, X09 ported). Total: 53->53 (depth improved, no new gaps). Suite 338/0/13. v453. | tests/test_provider_azure.c — expanded 54→94 assertions (+40), 4 new test functions. src/agent/provider_azure.c — bug fix (+3 lines). test_runner.sh — label: 54→94. |
| 398 | S7 X03 | Bedrock Provider Test Expansion — 61 new assertions in test_provider_bedrock.c (41→102 total). 6 new test functions: test_url_edge_cases (NULL base→us-east-1, empty model→claude-3-sonnet), test_stop_reason_mapping (all 6 Bedrock stop reasons: end_turn→stop, stop_sequence→stop, tool_use→tool_calls, max_tokens→length, content_filtered→content_filter, guardrail_intervened→content_filter, unknown→stop, missing→"), test_parse_response_edge_cases (no output with tokens, no message with tokens, empty content→NULL content, tool-only→empty text+tool count), test_bedrock_is_context_overflow (15 cases: 3 patterns validation+too_long, validation+exceeds+max_tokens, stream_error+too_long, plus non-matches: access denied, NULL, empty, validation only), test_bedrock_classify_error (10 cases: context_overflow, rate_limit 3 patterns, overloaded 4 patterns, unknown, NULL), test_parse_stream_edge_depth (NULL→", empty→"). No bug fix needed (stopReason→finish_reason mapping was already implemented). S7 X03 depth improved (Bedrock provider tests). S7 clusters: 18 (X03 improved). Total: 53->53 (depth improved, no new gaps). Suite 338/0/13. v454. | tests/test_provider_bedrock.c — expanded 41→102 assertions (+61), 6 new test functions. test_runner.sh — label: 40→102. |
| 399 | S7 X03 | OpenRouter Provider Test Expansion — 43 new assertions in test_provider_openrouter.c (51→94 total). 5 new test functions: test_url_edge_cases (double-slash fix, proxy path, empty base), test_header_edge_cases (empty/NULL key omits Authorization, long key included), test_parse_response_edge_cases (empty choices, no choices key, null content, length finish, no usage), test_parse_stream_reasoning (reasoning_content in delta captured), test_parse_stream_edge_depth (empty delta, empty chunk, length finish, whitespace, reasoning text). Bug fix: openrouter_parse_response — missing finish_reason extraction from choice level. S7 X03 depth improved (OpenRouter provider tests). S7 clusters: 18 (X03 improved, X04 improved). Total: 53->53 (depth improved, no new gaps). Suite 338/0/13. v455. | tests/test_provider_openrouter.c — expanded 51→94 assertions (+43), 5 new test functions. src/agent/provider_openrouter.c — bug fix (+3 lines). test_runner.sh — label: 51→94. |
| 400 | S7 X03 | Custom Provider Test Expansion — 35 new assertions in test_provider_custom.c (40→75 total). 4 new test functions: test_url_edge_cases (proxy path, custom model, empty base), test_header_edge_cases (empty/NULL key omits Authorization, long key included), test_parse_response_edge_cases (empty choices, no choices key, null content, length finish, no usage), test_parse_stream_edge_depth (empty delta, empty chunk, length finish, whitespace). Bug fix: custom_parse_response — missing finish_reason extraction from choice level. S7 X03 COMPLETE — all 10 provider test files expanded. S7 clusters: 18 (X03 100% provider coverage). Total: 53->53 (depth improved, no new gaps). Suite 338/0/13. v456. | tests/test_provider_custom.c — expanded 40→75 assertions (+35), 4 new test functions. src/agent/provider_custom.c — bug fix (+3 lines). test_runner.sh — label: 59→75. |
| 401 | S7 X04 | Clarify Tool Edge Case Expansion — 20 new assertions in test_clarify.c (39→59 total). 7 new test groups: unicode/emoji input, cancel input, very long response 2KB+, multi-line input, 10 choices, empty string question. Test harness fix: stdio buffer hygiene between piped stdin calls. S7 X04 depth improved (clarify edge cases +51%). S7 clusters: 18 (X04 improved). Total: 53->53 (depth improved, no new gaps). Suite 338/0/13. v457. | tests/test_clarify.c — expanded 39→59 assertions (+20), 7 new test groups, ~120 new LOC. test_runner.sh — label: clarify (59 assertions). |
| 402 | S7 X04 | exec_code Tool Edge Case Expansion — 6 new test functions in test_exec_code.c (15→21 total). New coverage: large output truncation (70KB+, truncated flag), negative timeout clamping (-1 clamps to default), extra fields ignored (unknown key/value pairs), quote injection (double+single quotes in code), task_id handling (non-NULL task_id), output field presence (exit_code/output/truncated fields verified). S7 X04 depth improved (exec_code edge cases +40%). S7 clusters: 18 (X04 improved). Total: 53->53 (depth improved, no new gaps). Suite 338/0/13. v458. | tests/test_exec_code.c — expanded 15→21 test functions (+6), ~120 new LOC. test_runner.sh — label: 15→21. |
| 403 | S7 X04 | Skills Hub Edge Case Expansion — 7 new test functions in test_skills_hub.c (17→41 tests). New coverage: empty catalog (skills:[] parses cleanly), missing skills key (no crash), single skill catalog (slug/name parsed), skill without tags (optional fields handle NULL), unicode skill names (café résumé preserved through JSON), null fields (name/title=NULL returns NULL, slug still works), long descriptions (1999-char desc preserved). S7 X04 depth improved (skills_hub edge cases +141%). S7 clusters: 18 (X04 improved). Total: 53->53 (depth improved, no new gaps). Suite 338/0/13. v459. | tests/test_skills_hub.c — expanded 17→41 tests (+24), 7 new test functions, ~250 new LOC. test_runner.sh — label: 17→41. |
| 404 | S7 X04 | TTS Tool Edge Case Expansion — 6 new test functions in test_tts_tool.c (11→17 tests). New coverage: speed clamping (0.5/2.0 min/max bounds, negative/zero clamped), provider validation (6 backends: espeak/edge/elevenlabs/openai/xai/azure, invalid/empty/case mismatch rejected), text with newlines (\\\\n preserved through escape), empty text error (empty/NULL rejected), very long text (2000-char preserved through escape), max chunk duration boundary (negative/zero rejected, 60/3600 accepted). S7 X04 depth improved (tts_tool edge cases +55%). S7 clusters: 18 (X04 improved). Total: 53->53 (depth improved, no new gaps). Suite 338/0/13. v460. | tests/test_tts_tool.c — expanded 11→17 test functions (+6), ~190 new LOC. test_runner.sh — label: 11→17. |
| 405 | S7 X04 | Discord Tool Edge Case Expansion — 6 new test functions in test_discord.c (13→19 tests). New coverage: empty action string ('\"\"'→Missing action field), fetch_messages missing channel_id, search_members missing guild_id/query, pin_message missing message_id, unpin_message missing channel_id, create_thread missing name, add_role missing role_id, remove_role missing user_id. Covers 6 previously untested Discord API actions. S7 X04 depth improved (discord_tool edge cases +46%). S7 clusters: 18 (X04 improved). Total: 53->53 (depth improved, no new gaps). Suite 338/0/13. v461. | tests/test_discord.c — expanded 13→19 test functions (+6), ~200 new LOC. test_runner.sh — label: 13→19. |
