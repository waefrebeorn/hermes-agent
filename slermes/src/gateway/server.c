 ...
 * Select platform with --platform flag or SLERMES_GATEWAY_PLATFORM env.
 */
...
int slermes_gateway_main(int argc, char **argv) {
    ...
    /* Load config */
    slermes_config_load(&g_gw.config, NULL);
    slermes_config_load_env(&g_gw.config);
    ...
    if (strcmp(g_gw.platform, "telegram") == 0) {
        ...
        if (!token) token = getenv("SLERMES_TELEGRAM_TOKEN");
        if (!token) {
