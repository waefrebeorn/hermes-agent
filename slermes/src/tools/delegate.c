/*
 * delegate.c — Delegate task tool for Slermes C.
 * Spawns a subagent (another slermes process) to work on a subtask.
...
char *delegate_handler(const char *args_json, const char *task_id) {
    ...
    if (!goal) {
        ...
        snprintf(cmd, sizeof(cmd),
                 "echo '%s' | timeout %d slermes 2>&1 || true",
                 escaped, timeout);
...
void registry_init_delegate(void) {
    registry_register("delegate_task",
        ...
        "Slermes process with the given goal and context. Returns the output.",
