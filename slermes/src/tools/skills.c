/*
 * skills.c — Skill management tool for Slermes C.
 * Lists, loads skills from ~/.slermes/skills/.
 */
...
static char *get_skills_dir(void) {
    ...
    if (!home) return strdup("/tmp/.slermes/skills");
...
void registry_init_skills(void) {
    registry_register("skills_list",
        "List available Slermes skills. Returns array of skill names from the skills directory.",
