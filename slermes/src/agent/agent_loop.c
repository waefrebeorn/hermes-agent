...
void agent_init(agent_state_t *state) {
    ...
    state->max_iterations = SLERMES_MAX_TOOL_CALLS;
    context_init(state);
...
bool agent_open_db(agent_state_t *state) {
    ...
    char db_dir[4096];
    snprintf(db_dir, sizeof(db_dir), "%s/sessions", state->slermes_home[0] ?
             state->slermes_home : (getenv("HOME") ? getenv("HOME") : "/tmp"));
                              ...
                              const char *system_message)
{
    ...
    while (iteration < state->max_iterations && !state->interrupted) {
        ...
        llm_truncate_context(state, SLERMES_MAX_CTX_TOKENS);
