 ...
 *
 * Phase 5 target: full slermes binary equivalent.
...
int main(int argc, char **argv) {
    ...
    if (argc > 1 && strcmp(argv[1], "gateway") == 0) {
        return slermes_gateway_main(argc - 1, argv + 1);
    }

    if (argc > 1 && strcmp(argv[1], "cron") == 0) {
        /* Forward declare — defined in scheduler.c */
        extern int slermes_cron_main(int, char**);
        return slermes_cron_main(argc - 1, argv + 1);
    }
    ...
    return slermes_cli_main(argc, argv);
}
