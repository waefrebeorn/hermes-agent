...
int slermes_cli_main(int argc, char **argv) {
    ...
    /* Load config */
    slermes_config_load(&g_cli.config, NULL);
    slermes_config_load_env(&g_cli.config);
    ...
    memcpy(g_cli.agent.slermes_home, g_cli.config.config_path,
           sizeof(g_cli.agent.slermes_home));
    /* Trim /config.yaml suffix */
    char *slash = strrchr(g_cli.agent.slermes_home, '/');
    if (slash) *slash = '\0';
