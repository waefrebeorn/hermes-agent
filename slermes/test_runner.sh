...
run_lib_test() {
    ...
    local bin="/tmp/slermes_test_${name}"
...
if gcc -O2 -Wall -Wextra -I"$CDIR/lib/libtemplate" -I"$CDIR/lib/libjson" \
    ...
    -o /tmp/slermes_test_template -lm > /dev/null 2>&1; then
    if /tmp/slermes_test_template > /dev/null 2>&1; then ok "template"
    else fail "template (test binary returned non-zero)"; fi
    rm -f /tmp/slermes_test_template
...
echo "  Building slermes binary..."
make -C "$CDIR" -j$(nproc) > /dev/null 2>&1 || {
    skip "slermes build"; summary; exit $FAIL
}
SLERMES="$CDIR/slermes"

# CLI: runs without crash
TIMEOUT_OUTPUT=$(timeout 2 "$SLERMES" 2>&1 || true)
...
# CLI: --version
if "$SLERMES" --version 2>&1 | grep -q "WuBu Slermes"; then
...
if "$SLERMES" gateway 2>&1 | grep -qi "platform"; then
...
test_webhook() {
    ...
    SLERMES_WEBHOOK_PORT=$port "$SLERMES" gateway --platform webhook &
    local pid=$!
    sleep 1
    local result
    cmd="${cmd//\$PORT/$port}"
