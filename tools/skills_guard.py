...
THREAT_PATTERNS = [
    ...
    (r'open\s*\([^)]*["\'][waxb+]',
     ...
     "Python open() with write/append/exclusive-create mode (may write files outside skill directory)"),
    (r'\.write_text\s*\(|\.write_bytes\s*\(',
     "pathlib_file_write", "medium", "persistence",
     "Path.write_text() or write_bytes() (file write via pathlib)"),
    (r'shutil\.(copy|move|copy2|copytree)\s*\(',
     "shutil_file_copy", "medium", "persistence",
     "shutil file copy/move operation"),
    (r'os\.rename\s*\(|os\.replace\s*\(',
     "os_file_rename", "medium", "persistence",
     "os file rename/replace operation"),
    ...
    # ── Additional Python execution vectors ──
    (r'subprocess\.(run|call|Popen|check_output)\s*\([^)]*shell\s*=\s*True',
     "python_subprocess_shell", "high", "execution",
     "subprocess with shell=True (command injection risk)"),
    (r'pickle\.(loads?|load)\s*\(',
     "pickle_deserialize", "critical", "execution",
     "pickle deserialization (arbitrary code execution via __reduce__)"),
    (r'ctypes\.(CDLL|LibraryLoader|cdll|WinDLL|windll)\s*\(',
     "ctypes_native_code", "high", "execution",
     "ctypes native code loading (C library injection)"),
    (r'importlib\.(import_module|util\.spec_from)',
     "importlib_dynamic", "medium", "execution",
     "dynamic module import (possible runtime dependency injection)"),
    (r'marshal\.(loads?|load)\s*\(',
     "marshal_deserialize", "critical", "execution",
     "marshal bytecode deserialization (arbitrary code execution)"),
    (r'os\.execlp?e?\s*\(|os\.execv[e]?\s*\(|os\.execvp[e]?\s*\(',
     "os_exec_replace", "high", "execution",
     "os.exec* process replacement (may bypass security monitoring)"),
    (r'dill\.(loads?|load)\s*\(|cloudpickle\.(loads?|load)\s*\(|jsonpickle\.(decode|loads)\s*\(',
     "alt_pickle_deserialize", "critical", "execution",
     "alternative pickle library deserialization (Dill, cloudpickle, jsonpickle)"),

    # ── Python persistence / environment hijack ──
    (r'os\.environ\s*\[.*LD_PRELOAD',
     "ld_preload_hijack", "critical", "persistence",
     "LD_PRELOAD via environment (library injection into all processes)"),
    (r'os\.environ\s*\[.*PYTHONPATH',
     "pythonpath_hijack", "high", "persistence",
     "PYTHONPATH via environment (import hijacking)"),
    (r'os\.chmod\s*\([^)]*(?:0o?[0-7]*[4-7][0-7]?|(?:stat\.)?S_ISUID)',
     "chmod_suid", "high", "persistence",
     "setting SUID bit via Python (privilege escalation)"),
    (r'os\.setuid\s*\(|os\.setgid\s*\(|os\.setresuid\s*\(',
     "python_setuid", "high", "persistence",
     "attempting to change user/group ID (privilege escalation)"),
    ...
    (r'urllib\.request\.urlopen\s*\(',
     "urllib_fetch", "medium", "supply_chain",
     "urllib URL fetch at runtime"),
    (r'urllib3\.(request|get|post|put)\s*\(',
     "urllib3_fetch", "medium", "supply_chain",
     "urllib3 URL fetch at runtime"),
    (r'aiohttp\.ClientSession\s*\(',
     "aiohttp_fetch", "medium", "supply_chain",
     "aiohttp client session (HTTP request capability)"),
    (r'smtplib\.SMTP\s*\(',
     "smtp_connection", "medium", "supply_chain",
     "SMTP connection (email exfiltration vector)"),
    (r'git\s+clone\s+',
