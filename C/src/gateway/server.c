...
static void gateway_send(const char *platform, const char *target, const char *text) {
    ...
    if (strcmp(platform, "telegram") == 0) {
        ...
        if (len > 4000) {
            ...
            chunk[4000] = '\0';
            telegram_send_message(g_gw.http, target, chunk, "Markdown", NULL, false);
            if (len > 4000)
                telegram_send_message(g_gw.http, target, text + 4000, "Markdown", NULL, false);
        } else {
            telegram_send_message(g_gw.http, target, text, "Markdown", NULL, false);
        }
