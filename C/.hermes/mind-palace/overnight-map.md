...
| 305 | S7 X09 template edge case expansion — 26 new assertions (9→35). Full coverage: plain text passthrough, empty/NULL compile, default values (present+missing), for loop (3-item, empty), if (true/false/else), nested for-in-if, NULL safety (render/free/vars), missing var empty, multi-context renders, no-var introspection, static passthrough. Suite 325/0/14. v369. |
| 306 | S7 X09 glob edge case expansion — 48 new assertions (22→70). Full coverage: empty/null patterns/paths, character classes [abc]/[!abc]/[a-z]/[0-9], hidden dotfiles via .*, multi-* patterns, trailing/leading *, ** at end, **/ alone, cross-/ behavior (fnmatch-dependent), ** after slash, regression patterns (-,+,_, spaces), glob_find edge cases (empty pattern, empty dir, ? single-char, dotfiles via .*, exact relative path, empty dir *). Suite 325/0/14. v370. |
| 307 | S7 X09 difflib edge case expansion — 59 new assertions (22→81). Full coverage: ratio swap consistency (abc/abd), single char (match+mismatch), unicode (café), whitespace differences, long identical, substring ratio > 0.5, unified_diff NULL/empty/both-NULL (header-only), large context (9999), zero context, single line change, trailing newlines (empty and add), two hunks (2+ @@ markers), simple_diff empty/empty, full add/delete, single line, 3 distant changes, trailing newline diff, long line change (short→long). Suite 325/0/14. v371. |
| 308 | S7 X09 enum edge case expansion — 31 new assertions (17→48). Full coverage: out-of-range OBO (value==count returns NULL, not crash), large 21-value enum with all positions round-trip verified (ENUM_NAME + ENUM_PARSE both exhaustive), empty string parse returns default, partial match parse returns default, all-valid-values round-trip in single assertion, macro isolation (color_t/single_t/large_t all coexist with correct counts), const correctness, switch round-trip all 4 colors. Suite 325/0/14. v372. |
|| 309 | S7 X09 video_mime edge case expansion — 44 new assertions (14→58). Full coverage: 15 case variants (.Mp4, .MP4, .mP4, .WebM, .WEBM, .Mov, .MOV, .AvI, .AVI, .Mkv, .MKV, .Mpeg, .MPEG, .Mpg, .MPG), 7 multi-dot edges (.tar.mp4 uses last extension, .mp4.bak→NULL, hidden .video.mp4, .video no ext, just ., .., .a.b.c.mp4), 9 path edge cases (empty string, just /, no dot absolute, space in name, underscore, hyphen, parent dir ../, 60-char name, numbers in name), 4 extension lengths (1/2/3/15 chars), 8 non-video format rejections (.jpg, .jpeg, .gif, .bmp, .txt, .pdf, .zip, .html). test_runner.sh count updated 14→58. | `tests/test_video_mime.c` — 58 assertions (14→58). `test_runner.sh` — count 14→58. Suite 325/0/14. v373. |
