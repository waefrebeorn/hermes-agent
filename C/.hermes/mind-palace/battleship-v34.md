...
| v332 | Fork diverged — C/ lives only on fork | Suite 335/0/0 | 85 tools | 98 CLI**
...
| 04 | R04 | gemini_native_adapter.py | 971 | Gemini native API format translation (contents[]/parts[]/functionDeclarations). C's provider_google.c covers core flow. **Phase 265 depth:** google_coerce_content_to_text() ported (extracts text from Gemini content: handles NULL/JSON_NULL → "", string → copy, array → join text parts, object with type==text → text field — from Python _coerce_content_to_text()). **Phase 264 depth:** google_is_native_base_url() ported (detects native Gemini API endpoint). **Phase 257 depth:** google_map_finish_reason() mapping + google_is_free_tier_quota_error() + blocked content detection. | P1 | PARTIAL |
