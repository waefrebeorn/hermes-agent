...
| v331 | Fork diverged — C/ lives only on fork | Suite 335/0/0 | 85 tools | 98 CLI**
...
| 04 | R04 | gemini_native_adapter.py | 971 | Gemini native API format translation (contents[]/parts[]/functionDeclarations). C's provider_google.c covers core flow. **Phase 264 depth:** google_is_native_base_url() ported (detects native Gemini API endpoint: contains generativelanguage.googleapis.com, doesn't end with /openai — from Python is_native_gemini_base_url()). **Phase 257 depth:** google_map_finish_reason() mapping (STOP/MAX_TOKENS/SAFETY/BLOCKLIST/etc → stop/length/content_filter). google_is_free_tier_quota_error() detects 429 free-tier exhaustion. Blocked content detection for finishReason=SAFETY. | P1 | PARTIAL |
