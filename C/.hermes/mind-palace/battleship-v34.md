...
| v339 | Fork diverged — C/ lives only on fork | Suite 335/0/0 | 85 tools | 98 CLI**
...
|| 02 | R02 | bedrock_adapter.py | 1289 | AWS Bedrock auth, model discovery. C has sigv4 signing + Converse API but simpler model/region handling. **Phase 271 depth:** 5 standalone utility functions ported: bedrock_is_anthropic_model() (regional prefix stripping + anthropic.claude matching), bedrock_model_supports_tool_use() (5 denylist patterns: deepseek.r1, deepseek-r1, stability., cohere.embed, amazon.titan-embed), bedrock_resolve_auth_env_var() (5 AWS env vars chain check), bedrock_has_credentials() (env-only wrapper), bedrock_resolve_region() (AWS_REGION/AWS_DEFAULT_REGION with us-east-1 fallback). 62 new tests (80→142). **Phase 263 depth:** 4 utility functions ported: bedrock_is_context_overflow() (3 pattern groups, case-insensitive strstr), bedrock_classify_error() (context_overflow/rate_limit/overloaded/unknown), bedrock_extract_provider_from_arn() (foundation-model/provider parsing), bedrock_get_context_length() (18-entry substring table, longest match wins). **Phase 262 depth:** stop_reason mapping (end_turn/stop_sequence→stop, tool_use→tool_calls, max_tokens→length, content_filtered/guardrail_intervened→content_filter). 142-test bedrock depth suite (up from 14). | P1 | PARTIAL |
...
|| 10 | R10 | model_metadata.py | 1850 | Model discovery, catalog, capabilities — **Phase 271 depth:** 6 functions ported: model_grok_supports_reasoning_effort() (3 grok prefix checks), provider_is_openrouter_base_url() (URL substring check), provider_is_custom_endpoint() (non-OpenRouter valid endpoint), provider_is_known_base_url() (wraps infer_from_url), provider_auth_headers() (json_t Authorization: Bearer builder), provider_coerce_reasonable_int() (string-to-int with comma-strip, bounds check, endptr UAF fix). 183-test suite (up from 141). Prior: 9 functions (provider_normalize_base_url, provider_strip_prefix, provider_is_local_endpoint, provider_infer_from_url, provider_parse_context_limit_from_error, provider_parse_available_output_tokens_from_error, provider_model_id_matches, provider_model_suggests_kimi, provider_normalize_model_version). 141-test suite. | P1 | PARTIAL (15/43 = 35%) |
