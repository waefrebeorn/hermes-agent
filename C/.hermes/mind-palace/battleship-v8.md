...
Total: **226 active gaps** across 22 sectors (resolved items retired to vault Phase 10+).
...
## SECTOR 11: Bug Fixes & Known Issues (9 gaps)
...
| 3 | B09 | Session metadata not saved on /title | commands.c (cmd_title checks agent_save_meta) | 10 | P2 |
| 4 | B11 | Gateway platforms don't validate config on startup | server.c | 50 | P2 |
| 5 | B12 | Cron jobs don't inherit gateway session context | cron/scheduler.c | 60 | P2 |
| 6 | B13 | Skill hub fails silently on network error | skills_hub.c | 20 | P2 |
| 7 | B14 | Plugin load errors not propagated to user | plugin.c | 30 | P2 |
| 8 | B15 | No log rotation for gateway logs | gateway/server.c | 40 | P2 |
| 9 | B16 | OOM not handled in `db_list_with_meta` loop | db.c:522 (calloc NULL check) | 5 | P3 |
