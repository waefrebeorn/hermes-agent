...
Total: **235 active gaps** across 22 sectors (resolved items retired to vault Phase 10+).
...
## SECTOR 9: Configuration & Environment (10 gaps)
...
| 3 | C12 | credentials.sources | Multi-source credential resolution order | 25 | P2 |
| 4 | C13 | gateway.signal.* | Signal-specific config (number, etc.) | 20 | P2 |
| 5 | C14 | proxy.https_proxy | Separate HTTPS proxy config | 10 | P2 |
| 6 | C15 | proxy.no_proxy | Per-host proxy bypass list | 15 | P2 |
| 7 | C17 | agent.checkpoint.* | Checkpoint config | 30 | P3 |
| 8 | C18 | agent.vault.* | Vault encryption config | 20 | P2 |
| 9 | C19 | cron.scheduler.* | Cron scheduler config | 30 | P2 |
| 10 | C20 | tools.browser.* | Browser-specific config (timeout, user-agent, etc.) | 20 | P2 |
...
## SECTOR 11: Bug Fixes & Known Issues (10 gaps)
...
| 4 | B09 | Session metadata not saved on /title | commands.c (cmd_title checks agent_save_meta) | 10 | P2 |
| 5 | B11 | Gateway platforms don't validate config on startup | server.c | 50 | P2 |
| 6 | B12 | Cron jobs don't inherit gateway session context | cron/scheduler.c | 60 | P2 |
| 7 | B13 | Skill hub fails silently on network error | skills_hub.c | 20 | P2 |
| 8 | B14 | Plugin load errors not propagated to user | plugin.c | 30 | P2 |
| 9 | B15 | No log rotation for gateway logs | gateway/server.c | 40 | P2 |
| 10 | B16 | OOM not handled in `db_list_with_meta` loop | db.c:522 (calloc NULL check) | 5 | P3 |
