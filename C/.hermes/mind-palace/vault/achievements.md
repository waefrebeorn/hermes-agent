...
| P218-02 | L28 agent init reclassified PORTED. Implemented tool_delay (1.0s default, usleep between tool iterations). agent_init() + agent_configure_from_config() cover all core init features: model/provider/credentials, toolsets, compression, checkpoints, budget, session, fallback, guardrails, prefill, steer, memory/skill nudges, background review, reasoning, service tier. Remaining fields are Python-arch-specific (callbacks, print_fn, custom provider helpers) — WON'T PORT. S1 complete: L24+L25+L26+L27+L28 PORTED. | Evidence: agent_loop.c:57 agent_init(), agent_loop.c:106 agent_configure_from_config(), hermes.h state->tool_delay, agent_loop.c usleep(). |
Suite: 325/0/0 (282 test files). Gaps: 104. v284
