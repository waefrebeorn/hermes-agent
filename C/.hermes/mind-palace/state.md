...
|||- Phase 150: G02 base.py depth — media_cache_save() + media_cache_cleanup() ported from Python gateway/platforms/base.py. New media_cache.c module saves/cleans cached media files under ~/.hermes/cache/. 15-test suite. Suite 302/0/0 (v226).
|- Phase 151: G09 yuanbao_media depth — url_get_image_format() + url_parse_image_size() ported from Python gateway/platforms/yuanbao_media.py. url_get_image_format maps 8 MIME types to TIM format numbers. url_parse_image_size extracts PNG/JPEG/GIF/WebP dimensions from raw bytes via magic byte detection. URL safety tests 68→83 (+15). Header: hermes_url_safety.h. Suite 302/0/0 (v226).
|- Phase 152: G02 base.py depth — media_should_send_as_audio() ported from Python gateway/platforms/base.py. Audio routing helper with 6 audio extension constants. Telegram-specific rules for voice vs audio attachment. Media cache tests 15→32 (+17). Suite 302/0/0 (v226).
|- Phase 153: G02 base.py depth — http_parse_retry_after() ported from Python httpx/http.client Retry-After handling. Parses integer seconds and HTTP-date format. Returns delay in seconds for rate-limit retry timing. HTTP tests +7. Suite 302/0/0 (v226).
